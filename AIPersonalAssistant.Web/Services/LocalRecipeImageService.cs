...
public class LocalRecipeImageService : IRecipeImageService
{
    private readonly string _imageDirectory;
    private const long MaxFileSizeBytes = 2 * 1024 * 1024;

    public LocalRecipeImageService(IWebHostEnvironment environment)
    {
        _imageDirectory = Path.Combine(environment.ContentRootPath, "App_Data", "recipe-images");
    ...
    public async Task<string> SaveImageAsync(string recipeId, Stream imageStream, string fileName)
    {
        ...
        var recipeDir = Path.Combine(_imageDirectory, recipeId);
        if (!Directory.Exists(recipeDir))
        {
            Directory.CreateDirectory(recipeDir);
        }

        var filePath = Path.Combine(recipeDir, $"{imageId}.jpg");
    ...
    public Task DeleteImageAsync(string recipeId, string imageId)
    {
        var filePath = Path.Combine(_imageDirectory, recipeId, $"{imageId}.jpg");
    ...
    public Task<(Stream stream, string contentType)?> GetImageAsync(string recipeId, string imageId)
    {
        var filePath = Path.Combine(_imageDirectory, recipeId, $"{imageId}.jpg");
        if (!File.Exists(filePath))
        {
    ...
    public Task DeleteAllImagesAsync(string recipeId)
    {
        var recipeDir = Path.Combine(_imageDirectory, recipeId);
        if (Directory.Exists(recipeDir))
        {
            Directory.Delete(recipeDir, true);
        }
        return Task.CompletedTask;
    }
